...
// Computes a vertical scrollbar thumb's position and size within a
// track, given how much content there is vs. how much is visible and
// the current scroll offset — shared by any panel with more content
// than fits (the search results panel, the hover-preview text view).
// Returns a thumb filling the whole track when all the content already
// fits (nothing to scroll), so callers can use "thumb.height ==
// trackHeight" as the "don't bother drawing a scrollbar" signal.
inline ScrollbarThumb ComputeScrollbarThumb(
    int contentHeight,
    int viewportHeight,
    int scrollOffset,
    int trackHeight,
    int minThumbHeight)
{
    if (contentHeight <= viewportHeight || contentHeight <= 0 || trackHeight <= 0)
        return { 0, trackHeight };

    int thumbHeight =
        (int)((double)viewportHeight / (double)contentHeight * trackHeight);
    ...
    int thumbTop =
        maxScroll > 0
            ? (int)((double)scrollOffset / (double)maxScroll * maxThumbTop)
            : 0;
...
// Inverse of the thumb-top half of ComputeScrollbarThumb: given a
// thumb-top position (e.g. while dragging, or a click on the track),
// returns the scroll offset that would produce it.
inline int ScrollOffsetFromThumbTop(
    int thumbTop,
    int contentHeight,
    int viewportHeight,
    int trackHeight,
    int thumbHeight)
{
    ...
    int scroll =
        (int)((double)thumbTop / (double)maxThumbTop * maxScroll);
...
// ============================================================
// Extensionless-file text sniffing
// ============================================================

// Deliberately generous rather than strict: a file with no extension
// (README, LICENSE, Makefile, a dotfile) gets no help from its name, so
// this is what decides whether to still attempt a text preview instead
// of just giving up. A single NUL byte is still treated as a hard
// binary signal (real text essentially never contains one), but
// scattered control characters are tolerated up to a fairly loose
// fraction — the goal is "assume text unless there's good reason not
// to," not "reject at the first oddity," since guessing wrong just
// costs a quick, harmless glance at some garbled bytes (see
// classicshell.cpp's preview-loading comments for the same philosophy
// applied to the size caps).
inline bool LooksLikeText(
    ...
    size_t length)
{
    ...
    size_t sampleLen = length < 8192 ? length : 8192;
    size_t suspicious = 0;
    ...
    // Up to 15% odd bytes tolerated — generous on purpose (see above).
    return suspicious * 100 <= sampleLen * 15;
}
