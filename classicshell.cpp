    ...
    HWND hwnd)
{
    ...
    // so only the alpha byte needs adding on top. Blended with the
    // live accent color (same MixColor blend g_hot uses) rather than
    // a fixed panel color, so the glass actually tracks the user's
    // chosen Windows accent instead of looking the same regardless
    // of theme.
    DWORD tint =
        (DWORD)MixColor(g_panel, g_accent, 22) &
        0x00FFFFFF;
